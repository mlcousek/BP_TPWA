...
namespace BP_TPWA.Areas.Identity.Pages.Account.Manage
{
    public class DeletePersonalDataModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGet()
        {
            ...
            if (user == null)
            {
                return NotFound($"Nelze najít uživatele s ID '{_userManager.GetUserId(User)}'.");
            }
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!result.Succeeded)
            {
                throw new InvalidOperationException($"Nečekaný error při mazání uživatele.");
            }

            await _signInManager.SignOutAsync();

            _logger.LogInformation("Uživatel s ID '{UserId}' se smazal.", userId);
